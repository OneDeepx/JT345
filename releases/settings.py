...
class Settings:
    ...
    def save_api_key(self, service: str, key: str, secret: str = None):
        ...
        try:
            # Remove read-only attribute if file exists (Windows/OneDrive issue)
            if self.api_keys_file.exists():
                try:
                    os.chmod(self.api_keys_file, 0o600)  # Make writable
                except:
                    pass
            ...
            # Set read-only after writing (security)
            try:
                os.chmod(self.api_keys_file, 0o400)
            except:
                pass  # If chmod fails (Windows), continue anyway
        except PermissionError as e:
            raise PermissionError(
                f"Cannot write to {self.api_keys_file}. "
                "If the file is in OneDrive, try moving the bot folder to a local drive (like C:\\crypto-bot)."
            ) from e
